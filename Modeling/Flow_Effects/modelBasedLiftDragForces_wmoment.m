function [tau_lift_and_drag, alpha, F_lift, F_drag ,U_r] = modelBasedLiftDragForces_wmoment(nu_linkAC, vel_flow, rho, foil_param)
...
%     implement input for e_Oswald and parasitic drag
    ...
    U_vec = nu_linkAC(1:2);
    U_c_vec = vel_flow(1:2);
    U_r_vec = U_vec - U_c_vec;
    U_r = sqrt(U_r_vec(1)^2 + U_r_vec(2)^2); % relative speed
...
%     C_D = C_D_parasitic + ((C_L_0 + C_L_alpha*alpha)^2)/(pi*e_OS*AR); % if this model in linear, the drag will be negative for negative angle of attack!!
    ...
    M_foil = (1/2)*rho*(U_r^2)*S*foil_param.L*C_M;
...
%                     0,  1];
    ...
    tau_lift_and_drag = [XY_link; 0; 0; 0; -M_foil]; % 6DOF 
